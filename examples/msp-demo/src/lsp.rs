...
use elara_diffusion::{InterestLevel, LivestreamSwarm, SwarmConfig};
use elara_visual::{
    EmotionVector, FaceState, GazeState, MouthState, PredictionConfig, Viseme, VisualEncoder,
    VisualPredictor, VisualState,
};
    ...
    fn decode(data: &[u8]) -> Option<Self> {
        ...
        match data[0] {
            0x01 => {
                ...
                let broadcaster =
                    String::from_utf8_lossy(&data[3 + title_len..3 + title_len + broadcaster_len])
                        .to_string();
...
impl Broadcaster {
    ...
    fn generate_visual_state(&mut self) -> VisualState {
        ...
        face.gaze = GazeState {
            ...
            blink: if (self.face_phase % 4.0) < 0.15 {
                1.0
            } else {
                0.0
            },
        ...
        if is_keyframe {
            VisualState::keyframe(self.node_id, timestamp, self.sequence).with_face(face)
        } else {
            VisualState::delta(
                self.node_id,
                timestamp,
    ...
    async fn broadcast_viewer_count(&self) -> io::Result<()> {
        let msg = LspMessage::ViewerCount {
            count: self.viewers.len() as u32,
        };
        let encoded = msg.encode();
    ...
    fn render_ui(&self) {
        ...
        if !self.viewers.is_empty() {
            println!("╠══════════════════════════════════════════════════════════════╣");
            println!("║ Viewers:");
            for viewer in self.viewers.values().take(5) {
                println!(
                    "║   {} ({:?}, {}s)",
                    viewer.name,
                    viewer.interest,
                    viewer.joined_at.elapsed().as_secs()
                );
            }
        }
...
impl Viewer {
    ...
    async fn join_stream(&self) -> io::Result<()> {
        let msg = LspMessage::ViewerJoin {
            name: self.name.clone(),
        };
        self.socket
            .send_to(&msg.encode(), self.broadcaster_addr)
            .await?;
        Ok(())
    }
        ...
        match msg {
            ...
            LspMessage::VisualUpdate { data, sequence } => {
                if let Ok(state) = VisualEncoder::decode(&data) {
                    let degraded = state.degrade(self.degradation);
                    self.predictor.update(degraded);
...
async fn main() -> io::Result<()> {
    ...
    if is_broadcaster {
        ...
        let broadcaster_addr: SocketAddr = addr_str.trim().parse().expect("Invalid address");
