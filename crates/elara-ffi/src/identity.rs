...
#[no_mangle]
pub extern "C" fn elara_identity_free(handle: *mut ElaraIdentityHandle) {
    if !handle.is_null() {
        unsafe {
...
#[no_mangle]
pub extern "C" fn elara_identity_node_id(handle: *const ElaraIdentityHandle) -> ElaraNodeId {
    if handle.is_null() {
...
#[no_mangle]
pub extern "C" fn elara_identity_public_key(
...
#[no_mangle]
pub extern "C" fn elara_identity_export(handle: *const ElaraIdentityHandle) -> ElaraBytes {
    if handle.is_null() {
        set_last_error("Null pointer");
...
/// Returns a handle that must be freed with elara_identity_free
#[no_mangle]
pub extern "C" fn elara_identity_import(
    data: *const u8,
    len: usize,
...
mod tests {
    ...
    fn test_identity_generate_free() {
        ...
        let node_id = elara_identity_node_id(handle);
    ...
    fn test_identity_export_import() {
        let handle = elara_identity_generate();
        let original_id = elara_identity_node_id(handle);
        
        let exported = elara_identity_export(handle);
        assert!(!exported.is_empty());
        
        let imported = elara_identity_import(exported.data, exported.len);
        assert!(!imported.is_null());
        
        let imported_id = elara_identity_node_id(imported);
        assert_eq!(original_id.value, imported_id.value);
        
        elara_identity_free(handle);
        elara_identity_free(imported);
        
        // Free the exported bytes
        unsafe {
            if !exported.data.is_null() {
                drop(Vec::from_raw_parts(exported.data, exported.len, exported.capacity));
            }
        }
    }
}
