...
use crate::{
    FaceState, PoseState, SceneState, VisualState, VisualStateId,
    EmotionVector, GazeState, MouthState, Viseme,
    Position3D, Rotation3D, JointState, Joint,
    Color, LightingCondition, EnvironmentType, BackgroundComplexity,
};
...
impl VisualEncoder {
    ...
    pub fn encode(state: &VisualState) -> Vec<u8> {
        ...
        let mut flags: u8 = 0;
        if state.face.is_some() { flags |= 0x01; }
        if state.pose.is_some() { flags |= 0x02; }
        if state.scene.is_some() { flags |= 0x04; }
        buf.push(flags);
    ...
    fn encode_face(face: &FaceState, buf: &mut Vec<u8>) {
        // Face flags
        let mut flags: u8 = 0;
        if face.present { flags |= 0x01; }
        if face.speaking { flags |= 0x02; }
        buf.push(flags);
    ...
    fn encode_scene(scene: &SceneState, buf: &mut Vec<u8>) {
        ...
        // Flags
        let mut flags: u8 = 0;
        if scene.background_motion { flags |= 0x01; }
        buf.push(flags);
    ...
    pub fn decode(data: &[u8]) -> Result<VisualState, EncodingError> {
        ...
        let is_keyframe = data[pos] == 0x01; pos += 1;
        let degradation_level = data[pos]; pos += 1;
        pos += 1; // Reserved
        ...
        pos += 8;
        let keyframe_ref = if keyframe_ref_val == 0 { None } else { Some(VisualStateId(keyframe_ref_val)) };
