...
use crate::{
    VoiceState, VoiceStateId, VoiceParams, VoiceActivity, VoiceFrame,
    Formant, PitchContour, SpeechEmotion,
};
...
impl VoiceEncoder {
    ...
    pub fn decode_state(data: &[u8]) -> Result<VoiceState, VoiceEncodingError> {
        ...
        pos += 8;
        let keyframe_ref = if keyframe_ref_val == 0 { None } else { Some(VoiceStateId(keyframe_ref_val)) };
    ...
    pub fn decode_frame(data: &[u8]) -> Result<VoiceFrame, VoiceEncodingError> {
        ...
        let voiced = data[pos] == 0x01; pos += 1;
        let is_delta = data[pos] == 0x01; pos += 1;
