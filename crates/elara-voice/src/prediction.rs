...
use crate::{VoiceActivity, VoiceFrame, VoiceState, VoiceStateId};
...
impl VoicePredictor {
    ...
    pub fn update_state(&mut self, state: VoiceState) {
        ...
        if let (Some(prev), Some(prev_params), Some(curr_params)) = (
            &self.last_state,
            self.last_state.as_ref().and_then(|s| s.params.as_ref()),
            state.params.as_ref(),
        ) {
    ...
    pub fn predict(&mut self, target_time: StateTime) -> Option<VoiceState> {
        ...
        if let Some(ref mut params) = predicted.params {
            ...
            params.pitch = (params.pitch + self.pitch_velocity * frames_ahead).clamp(50.0, 500.0);
            params.energy = (params.energy + self.energy_velocity * frames_ahead).clamp(0.0, 1.0);
        ...
        if self.prediction_count > self.config.fade_out_frames {
            let fade = 1.0
                - (self.prediction_count - self.config.fade_out_frames) as f32
    ...
    pub fn predict_frame(&mut self, target_time: StateTime) -> Option<VoiceFrame> {
        ...
        // Apply velocity
        predicted.pitch = (predicted.pitch + self.pitch_velocity * frames_ahead).clamp(50.0, 500.0);
        predicted.energy = (predicted.energy + self.energy_velocity * frames_ahead).clamp(0.0, 1.0);
