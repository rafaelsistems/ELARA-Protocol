...
use crate::{VoiceActivity, VoiceParams, VoiceStateId, SpeechEmotion};
...
impl VoiceFrame {
    ...
    pub fn voiced(source: NodeId, timestamp: StateTime, sequence: u64, pitch: f32, energy: f32) -> Self {
        Self {
            sequence,
            timestamp,
            source,
    ...
    pub fn from_params(source: NodeId, timestamp: StateTime, sequence: u64, params: &VoiceParams) -> Self {
        Self {
            sequence,
            timestamp,
            source,
    ...
    fn params_to_spectral_index(params: &VoiceParams) -> u16 {
        ...
        let index = (f1_ratio * 255.0) as u16 | ((f2_ratio * 255.0) as u16) << 8;
        index
    }
    ...
    pub fn lerp(&self, other: &VoiceFrame, t: f32) -> VoiceFrame {
        ...
        VoiceFrame {
            sequence: if t < 0.5 { self.sequence } else { other.sequence },
            ...
            spectral_index: if t < 0.5 { self.spectral_index } else { other.spectral_index },
            noise_seed: rand::random(),
            is_delta: false,
        }
    }
